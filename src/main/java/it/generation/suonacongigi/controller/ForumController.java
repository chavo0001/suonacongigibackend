...
public class ForumController extends BaseController {
    ...
    public ResponseEntity<ApiEnvelope<List<CategoryResponse>>> getCategories() {
        // Il controller si limita a delegare la logica di recupero delle categorie al
        // servizio,
    ...
    // Endpoint pubblico per ottenere la lista dei thread appartenenti a una
    // specifica categoria tramite ID.
    ...
    public ResponseEntity<ApiEnvelope<List<ThreadSummaryResponse>>> getThreads(@PathVariable Long id) {
        // Il controller si limita a delegare la logica di recupero dei thread al
        // servizio,
        // restituendo un ResponseEntity con il risultato.
    ...
    public ResponseEntity<ApiEnvelope<ThreadDetailResponse>> getThread(
            @Parameter(description = "ID del thread da recuperare", example = "42") @PathVariable Long id,
            @AuthenticationPrincipal User user) {
        // Se l'utente è autenticato, passiamo il suo username al servizio per
        // verificare i permessi.
        String username = (user != null) ? user.getUsername() : null;

        // Il controller si limita a delegare la logica di recupero dei dettagli del
        // thread al servizio,
        // restituendo un ResponseEntity con il risultato.
    ...
    // Endpoint protetto per creare una nuova discussione (thread) in una categoria
    // specifica.
    // Accettando un DTO con i dati del thread, valida i dati e delega la logica al
    // servizio.
    ...
    public ResponseEntity<ApiEnvelope<ThreadSummaryResponse>> createThread(
            ...
            @AuthenticationPrincipal User user) {
        // @Valid: Innesca il motore di validazione Bean Validation via Reflection sui
        // metadati del DTO.
        String username = Objects.requireNonNull(Objects.requireNonNull(user).getUsername());
        // Il controller si limita a delegare la logica di creazione del thread al
        // servizio,
        // restituendo un ResponseEntity con il risultato.
        // La validazione dei dati avviene automaticamente grazie alle annotazioni di
        // validazione sui campi del DTO.
    ...
    // Accetta un DTO con i dati del post, valida i dati e delega la logica al
    // servizio.
    ...
    public ResponseEntity<ApiEnvelope<PostResponse>> addPost(
            @Parameter(description = "ID del thread a cui rispondere", example = "42") @PathVariable Long id,
            @Valid @RequestBody PostRequest req,
            @AuthenticationPrincipal User user) {
        // @Valid: Innesca il motore di validazione Bean Validation via Reflection sui
        // metadati del DTO.
        String username = Objects.requireNonNull(Objects.requireNonNull(user).getUsername());
        // Il controller si limita a delegare la logica di aggiunta del post al
        // servizio,
        // restituendo un ResponseEntity con il risultato.
        // La validazione dei dati avviene automaticamente grazie alle annotazioni di
        // validazione sui campi del DTO.
    ...
    public ResponseEntity<ApiEnvelope<Void>> deletePost(
            @Parameter(description = "ID del post da eliminare", example = "123") @PathVariable Long id,
            @AuthenticationPrincipal User user) {
        // Solo l'autore del post o un amministratore possono eliminarlo
        ...
        // Se il servizio non ha lanciato eccezioni, significa che l'eliminazione è
    ...
    public ResponseEntity<ApiEnvelope<Void>> deleteThread(
            @Parameter(description = "ID del thread da eliminare", example = "42") @PathVariable Long id,
            @AuthenticationPrincipal User user) {
