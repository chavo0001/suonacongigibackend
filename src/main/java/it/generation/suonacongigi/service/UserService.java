...
public class UserService {
    ...
    public void updateCollections(MusicalProfile profile, MusicalProfileRequest req) {
        if (req.getGenreIds() != null) {
            syncCollection(Objects.requireNonNull(req.getGenreIds()), Objects.requireNonNull(genreRepository),
                    Objects.requireNonNull(profile.getGenres()));
        }
        if (req.getInstrumentIds() != null) {
            syncCollection(Objects.requireNonNull(req.getInstrumentIds()), Objects.requireNonNull(instrumentRepository),
                    Objects.requireNonNull(profile.getInstruments()));
        }
        if (req.getArtistIds() != null) {
            syncCollection(Objects.requireNonNull(req.getArtistIds()), Objects.requireNonNull(artistRepository),
                    Objects.requireNonNull(profile.getFavoriteArtists()));
        }
    }
    ...
    private UserResponse toResponse(User user) {
        MusicalProfileResponse mpr = profileRepository.findByUserId(user.getId())
                ...
                .orElse(MusicalProfileResponse.builder() // ← orElse, NON orElseThrow
                        .bio(null)
                        .genres(Set.of())
                        .instruments(Set.of())
    ...
    private MusicalProfileResponse toMusicalProfileResponse(MusicalProfile p) {
        return Objects.requireNonNull(MusicalProfileResponse.builder()
                ...
                .favoriteArtists(
                        p.getFavoriteArtists().stream().map(this::toArtistResponse).collect(Collectors.toSet()))
                .build());
    }
